	...
	WebDriver driver;
	  ...
	  public void DiscountTest() {
		  ...
		  for(String handle:driver.getWindowHandles()) {
		  	...
		  	String vSubTotal=driver.findElement(By.xpath(".//*[@id='shopping-cart-totals-table']/tbody/tr[1]/td[2]/span")).getText().trim();
		  	System.out.println("Subtotal is :" +vSubTotal);
		  	String vDisPercent=driver.findElement(By.xpath(".//*[@id='shopping-cart-totals-table']/tbody/tr[2]/td[2]/span")).getText().trim();
		  	System.out.println("Discount pecent is :"+vDisPercent);
		  	String vDisplayAmount=driver.findElement(By.xpath(".//*[@id=\'shopping-cart-totals-table\']/tfoot/tr/td[2]/strong/span")).getText().trim();
		  	System.out.println("Display Amount is :"+vDisplayAmount);
		  	
		  	//Remove the $ sign
		  	String sSubTotal =vSubTotal.replaceAll("\\$", " ");
		  	String sDisPercent=vDisPercent.replaceAll("\\$"," ");
		  	String sDisplayAmount=vDisplayAmount.replaceAll("\\$", " ");
		  	//Remove -ve sign in discount percentage
		  	String sDiscount=sDisPercent.replaceAll("\\-", " ");
		  	
		  	//Now trim the spaces from values
		  	
		  	String fSubTotal =sSubTotal.trim();
		  	String fDiscount=sDiscount.trim();
		  	String fDisplayAmount = sDisplayAmount.trim();
		  	System.out.println(fSubTotal + "\n"+ fDiscount + "\n" + fDisplayAmount);
	    ...
	    // ***************************************************************************************************************************		    
	  	...
	  	double dSubTotal = Double.parseDouble(fSubTotal);
	  	
	  	double dDiscount =Double.parseDouble(fDiscount);
		
	  	double dDisplayAmount = Double.parseDouble(fDisplayAmount);
	    
	    // multiply the dSubTot by the GURU50 discount rate (GURU50 = 5% = 0.05) 
	    double discountedAmt = (dSubTotal * GURU50);     // discountedAmt is the calculated discounted amount (e.g. $25.00)	
	    double dDiscPrice = (dSubTotal - discountedAmt); // e.g. Discounted Price (e.g. $475) = Sub Total ($500.00) less the 5% discount amount ($25.00)
	 ...
	 // ************************************************************************************************
	    ...
	    if(discountedAmt == dDiscount) {
	    	...
	    	System.out.println("*** Displayed discount percent " + dDiscount);
	    	}else {
	    		System.out.println("Derived discount is not equal to displayed discount");
	    	}
	    //Asserting the discount values
	    
	    String string_discountedAmt = Double.toString(discountedAmt);
	    String string_dDiscount = Double.toString(dDiscount);	
	    try {
			assertEquals(string_discountedAmt, string_dDiscount);
		} catch (Exception e) {
			System.out.println("Derived discount is not equal to displayed discount");
			e.printStackTrace();
		}
	    
	    if(dDiscPrice==dDisplayAmount) {
	    	System.out.println("Derived final amount" +dDiscPrice);
	    	System.out.println("Displaye final Amount" +dDisplayAmount);
	    	
	    }else {
	    	System.out.println("Displaye final Amount is not equal to derived final amount");	    
	  }
	  
	  //Asserting the amount values
	  
	  String string_dDiscPrice =Double.toString(dDiscPrice);
	  String string_dDisplayAmount=Double.toString(dDisplayAmount);
	  try {
		assertEquals(string_dDiscPrice,string_dDisplayAmount);
	} catch (Exception e) {
		System.out.println("Displaye final Amount is not equal to derived final amount");
		e.printStackTrace();
	}
